      ...
      {bookings.length === 0 ? (
        ...
        <div style={{ background: 'var(--bg-card, var(--bg-card))', borderRadius: 12, border: '1px solid var(--border, var(--border))', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ background: 'var(--bg-card-deep, var(--bg-input))', borderBottom: '1px solid var(--border, #2A2A3A)' }}>
                <th style={{ padding: '12px 16px', fontSize: 13, color: 'var(--text-muted, var(--text-faint))', fontWeight: 500 }}>Date</th>
                <th style={{ padding: '12px 16px', fontSize: 13, color: 'var(--text-muted, var(--text-faint))', fontWeight: 500 }}>User</th>
                <th style={{ padding: '12px 16px', fontSize: 13, color: 'var(--text-muted, var(--text-faint))', fontWeight: 500 }}>Travel</th>
                <th style={{ padding: '12px 16px', fontSize: 13, color: 'var(--text-muted, var(--text-faint))', fontWeight: 500 }}>Route</th>
                <th style={{ padding: '12px 16px', fontSize: 13, color: 'var(--text-muted, var(--text-faint))', fontWeight: 500 }}>Amount</th>
                <th style={{ padding: '12px 16px', fontSize: 13, color: 'var(--text-muted, var(--text-faint))', fontWeight: 500 }}>Status</th>
              </tr>
            </thead>
            <tbody>
              {bookings.map(b => (
                <tr key={b.id} style={{ borderBottom: '1px solid var(--border-soft, var(--bg-input))', transition: 'background .15s' }}>
                  ...
                  <td style={{ padding: '14px 16px', fontSize: 13, color: 'var(--text-body, var(--text-body))' }}>
                    {b.travel_mode}<br/>
                    <span style={{ fontSize: 11, color: 'var(--text-muted, var(--text-faint))' }}>PNR: {b.pnr_number}</span>
                  </td>
                  <td style={{ padding: '14px 16px', fontSize: 13, color: 'var(--text-body, var(--text-body))' }}>
                  ...
                  <td style={{ padding: '14px 16px', fontSize: 13, color: 'var(--success, var(--success))', fontWeight: 600 }}>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--text-muted, var(--text-faint))', fontWeight: 400 }}>Paid via Wallet</div>
                  </td>
                  <td style={{ padding: '14px 16px' }}>
                    <span style={{ background: 'color-mix(in srgb, var(--success) 8%, transparent)', color: 'var(--success, var(--success))', padding: '4px 8px', borderRadius: 4, fontSize: 11 }}>

...
function TicketsPanel({ tickets = [], documents = [] }) {
  ...
  if (!hasItems) return (
    <div style={{ padding:'16px 14px', background:'#1A1A22', borderRadius:8, fontSize:12, color:'#444', textAlign:'center' }}>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
      {tickets.map(t => (
        <div key={`t-${t.id}`} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 14px', background:'#1A1A22', borderRadius:8 }}>
          <div>
      ...
      {documents.map(d => (
        <div key={`d-${d.id}`} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 14px', background:'#1A1A22', borderRadius:8 }}>
          <div>
            <div style={{ fontSize:12, color:'#ccc' }}>
          ...
          <a href={docsAPI.download(d.id)} target="_blank" rel="noopener noreferrer"
             style={{ fontSize:11, background:'#0A84FF18', color:'#0A84FF', border:'1px solid #0A84FF30', padding:'4px 10px', borderRadius:6, textDecoration:'none', whiteSpace:'nowrap' }}>
            ⬇ Download
          </a>
        </div>
      ))}
    ...
    <div className="fade-up">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:24 }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <select value={filter} onChange={e=>setFilter(e.target.value)} style={{ background:'#1A1A22', border:'1px solid #2A2A35', borderRadius:8, color:'#E2E2E8', fontSize:12, padding:'7px 12px', outline:'none' }}>
            ...
            <thead>
              <tr style={{ borderBottom:'1px solid #1E1E2A' }}>
                {['ID','Employee','Route','Mode','Booking','Dates','Budget','Status','Tickets'].map(h => (
                  <th key={h} style={{ padding:'11px 13px', textAlign:'left', fontSize:10, color:'#3A3A4A', fontWeight:500, textTransform:'uppercase', letterSpacing:'.06em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(r => (
                <tr key={r.id}
                  onClick={() => selectRow(r)}
                  style={{ borderBottom:'1px solid #14141E', cursor:'pointer', background:selected?.id===r.id?'#1A1A26':'transparent', transition:'background .1s' }}
                  onMouseEnter={e=>{ if(selected?.id!==r.id) e.currentTarget.style.background='#161620' }}
                  onMouseLeave={e=>{ if(selected?.id!==r.id) e.currentTarget.style.background='transparent' }}>
                  <td style={{ padding:'11px 13px', fontSize:10, color:'#555', fontFamily:'monospace' }}>{r.id}</td>
                  <td style={{ padding:'11px 13px' }}>
                    <div style={{ fontSize:12, color:'#DDD', fontWeight:500 }}>{r.user_name}</div>
                    <div style={{ fontSize:10, color:'#555', marginTop:1 }}>{r.user_role}</div>
                  </td>
                  <td style={{ padding:'11px 13px', fontSize:12, color:'#888' }}>{r.from_location} → {r.to_location}</td>
                  ...
                  <td style={{ padding:'11px 13px' }}>
                    {r.approved_total ? (
                      ...
                      <div style={{ fontSize:11, color:'#555' }}>₹{Number(r.estimated_total).toLocaleString('en-IN')} est.</div>
                    )}
                  ...
                  <td style={{ padding:'11px 13px' }}>
                    {r.documents?.length > 0 && (
                      <span style={{ fontSize:10, background:'#0A84FF18', color:'#0A84FF', padding:'3px 8px', borderRadius:6 }}>
                        📎 {r.documents.length}
                      </span>
                    )}
                  </td>
            ...
            <div>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:18 }}>
                <div>
                  ...
                  <div style={{ fontSize:12, color:'#555', marginTop:3 }}>{detail.purpose} · {detail.from_location} → {detail.to_location} · {MODE_ICONS[detail.travel_mode]} {detail.travel_mode}</div>
                </div>
                <button onClick={() => { setSelected(null); setDetail(null) }} style={{ background:'none', border:'none', color:'#444', cursor:'pointer', fontSize:20 }}>✕</button>
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:18 }}>
                {[
                  ...
                  <div key={k} style={{ background:'#1A1A22', borderRadius:9, padding:14 }}>
              ...
              {detail.approvals?.length > 0 && (
                <div style={{ marginBottom:18 }}>
                  <div style={{ fontSize:11, color:'#3A3A4A', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:8 }}>Approval Trail</div>
                  {detail.approvals.map((a,i) => (
                    <div key={i} style={{ padding:'10px 12px', background: a.action==='rejected' ? '#FF453A0D' : '#1A1A22', border: a.action==='rejected' ? '1px solid #FF453A33' : '1px solid transparent', borderRadius:8, marginBottom:4 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                        <span style={{ fontSize:12, color:a.action==='approved'?'#30D158':'#FF453A' }}>{a.action==='approved'?'✓':'✗'}</span>
                        <span style={{ fontSize:12, color:'#ccc', flex:1 }}>{a.approver_name} <span style={{ color:'#555' }}>({a.approver_role})</span></span>
                        <span style={{ fontSize:10, color:'#444' }}>{new Date(a.acted_at).toLocaleString('en-IN')}</span>
                      </div>
                      {a.note && (
              ...
              {/* Tickets / Documents */}
              <div>
                <div style={{ fontSize:11, color:'#3A3A4A', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:10 }}>Tickets & Documents</div>

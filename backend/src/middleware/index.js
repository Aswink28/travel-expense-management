...
const authenticate = async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      'SELECT id,emp_id,name,email,mobile_number,role,department,avatar,color,reporting_to,is_active,ppi_wallet_id,approver_roles,approval_type,approval_flow,designation,tier_id FROM users WHERE id=$1',
      [decoded.userId]
    )

...
router.post('/search', async (req, res, next) => {
  try {
    ...
    const flights = await flightService.searchFlights(source, destination, date, passengers, travelClass)
...
router.post('/book-ticket', async (req, res, next) => {
  ...
  try {
    ...
    const { rows: requests } = await client.query('SELECT * FROM travel_requests WHERE id = $1 FOR UPDATE', [requestId]);
    ...
    if (!['approved', 'booking_in_progress'].includes(tr.status)) {
      return res.status(400).json({ success: false, message: `Cannot book. Request is in status: ${tr.status}` });
    }
    ...
    if (currentBalance < bookingAmount) {
      ...
      return res.status(402).json({
        ...
        data: { currentBalance, required: bookingAmount, shortfall: bookingAmount - currentBalance, employeeName: wallet[0].user_name || tr.user_name }
      });
    }
    ...
    const { rows: txnRow } = await client.query(
      ...
      [wallet[0].id, tr.user_id, requestId, price, `Flight booking (${fareType}) - ${selectedFlight.airline}`, adminId, newBal]
    )
    ...
    const { rows: booking } = await client.query(
      ...
      [requestId, wallet[0].id, adminId, tr.user_id, tr.from_location, tr.to_location, tr.start_date, price, pnr_number, selectedFlight.airline, txnRow[0].id]
    )
    ...
    const { rows: ticket } = await client.query(
      ...
      [booking[0].id, tr.user_id, requestId, pnr_number, booking[0].id, tr.user_name, tr.from_location, tr.to_location, tr.start_date,
       selectedFlight.departureTime || null, fareType || null, price, selectedFlight.airline,
       JSON.stringify({ flightId: selectedFlight.flightId, flightNumber: selectedFlight.flightNumber, departureTime: selectedFlight.departureTime, arrivalTime: selectedFlight.arrivalTime, duration: selectedFlight.duration, fareType })]
    )
    ...
    sendTicketEmail({
      ...
      selectedFlight,
      newBalance: newBal,
    }).catch(err => console.error('[Email] Failed to send ticket email:', err.message))
    ...
    try {
      ...
      await client2.query("UPDATE travel_requests SET status = 'approved' WHERE id = $1", [req.body.requestId])
      client2.release()
    } catch (_) {}
